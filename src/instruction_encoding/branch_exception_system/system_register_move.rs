...
#[inline(always)]
fn emit_system_register_move<P: InstructionProcessor<T>, T>(proc: &mut P, l: u8, o0: u8, op1: u8, crn: u8, crm: u8, op2: u8, rt: Register) -> T {
...
pub trait SystemRegisterMove<T>: InstructionProcessor<T> {
    ...
    /// ```
    #[inline(always)]
    fn msr_register(&mut self, op0: UImm2, op1: UImm3, crn: UImm4, crm: UImm4, op2: UImm3, xt: Register) -> T {
    ...
    /// ```
    #[inline(always)]
    fn mrs(&mut self, xt: Register, op0: UImm2, op1: UImm3, crn: UImm4, crm: UImm4, op2: UImm3) -> T {
        debug_assert!(op0 == 2 || op0 == 3, "op0 must be either 2 or 3, was {}", op0);
